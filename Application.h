...
void vcu_battery_frame_process(struct can_frame frame, float* batteryVoltage);
void ai_drive_request_frame_process(struct can_frame frame, float* frontTrq, float* rearTrq, float* steeringReq);
void vcu_wheel_speed_frame_process(struct can_frame frame, uint16_t* rr_wheelSpeed, uint16_t* rl_wheelSpeed, uint16_t* fr_wheelSpeed, uint16_t* fl_wheelSpeed);
int8_t process_motor_current_request(float frontCurrent, float rearCurrent, uint8_t* currentDataBuffer);
